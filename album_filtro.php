...
<html>
	...
	<body>
		...
		<?php
			...
			{
		?>
		<button id="apri_filtri">Filtri</button>
		<script>
			$(document).ready(function(){
				$("#div_filtri").slideToggle(1);
		...
		<div style="display:block;" id="div_filtri">
			<div class="copri"></div>
			<form class="filtri" method="get" action="album_filtro.php" id="get">
				<h3>Generi</h3>
				<?php
					...
					for ($genere = 0; $genere < count($genere_id); $genere ++)
					{
						...
						{
							echo '<lable for="genere_'.$genere_id[$genere].'">'.$nome_genere[$genere].' ('.$cont_genere[$genere].')</lable>'."\n";
							echo '<input type="checkbox" name="id_'.$genere.'" value="'.$genere_id[$genere].'" checked/><br/>';
						}
						else
						{
							echo '<lable for="genere_'.$genere_id[$genere].'">'.$nome_genere[$genere].' ('.$cont_genere[$genere].')</lable>'."\n";
							echo '<input type="checkbox" name="id_'.$genere.'" value="'.$genere_id[$genere].'"/><br/>';
						}
					}
				?>
				<hr/>
				...
				<?php
					...
					{
						echo '<label for="ordine">'.$ordinedesc["descrizione"][$ordi].'</lable>';
						if (isset($_GET["ordine"]))
						{
							if ($_GET["ordine"] == $ordinedesc["value"][$ordi])
							{
								echo '<input type="radio" value="'.$ordinedesc["value"][$ordi].'" name="ordine" checked/><br/>';
							}
							else
							{
								echo '<input type="radio" value="'.$ordinedesc["value"][$ordi].'" name="ordine"/><br/>';
							}
						}
						else
						{
						...
						<tbody>
							...
							<tr style="border: none; background-color: transparent;">
								...
								else
								{
									...
									{
										header ('Location: album_filtro.php?pagina=1');
									}
									else
									{
